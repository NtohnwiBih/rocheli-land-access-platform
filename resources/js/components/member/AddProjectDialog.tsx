...
import { Plus, CheckCircle2, ArrowLeft, ArrowRight, PauseCircle } from "lucide-react";
...
type CityData = { key: string; name_en: string; name_fr: string };

type InactiveSub = { id: number; label: string; plan_name: string };
        ...
        {atCap ? (
                ...
                {inactiveSubs.length === 0 ? (
                  ...
                  inactiveSubs.map((s) => (
                    <div key={s.id} className="flex items-center justify-between rounded-xl border border-border p-3">
                      ...
                      <Button variant="outline" size="sm" disabled={suspending} onClick={() => onSuspend(s.id)}>
          ...
          <>
            ...
            <DialogFooter className="sm:justify-between">
              <Button variant="ghost" onClick={step === 0 ? closeDialog : goBack} disabled={processing}>
                {step === 0 ? t("plans.cancel") : (<><ArrowLeft className="mr-1 h-4 w-4" /> Back</>)}
              </Button>
