  ...
  return (
    <>
      <Head title="Contact — Rocheli Real Properties">
        <meta name="description" content="Talk to a Rocheli advisor. Book a consultation, visit our offices, or chat on WhatsApp." />
        <meta property="og:title" content="Contact Rocheli Real Properties" />
        <meta property="og:description" content="Book a consultation or visit our office in Yaoundé" />
      </Head>
      ...
      <Breadcrumb
        ...
        title={
          <>
            {hero.title ?? "Let's talk about your"}{" "}
            <span className="italic text-gradient-gold">{hero.titleAccent ?? "first plot."}</span>
          </>
        }
        description={hero.description ?? "Book a 30-minute consultation with an advisor. Visit our office. Or ping us on WhatsApp — whichever works for you."}
      />
      ...
      <section className="py-16">
        <div className="container-x grid gap-8 lg:grid-cols-[1.2fr_1fr]">
          ...
          >
            <div>
              <h2 className="font-display text-3xl font-semibold">{form.title ?? "Send us a message"}</h2>
              <p className="mt-2 text-sm text-muted-foreground">{form.subtitle ?? "We'll respond within one business day."}</p>
            </div>
            ...
            <div>
              <label className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Interest</label>
          ...
          <div className="space-y-6">
            ...
            >
              ...
              <h3 className="mt-4 font-display text-2xl font-semibold">{booking.title ?? "Book an advisor"}</h3>
              <p className="mt-2 text-white/70 text-sm">{booking.body ?? "Pick a 30-minute slot with a Rocheli advisor. Available Mon–Sat."}</p>
              <div className="mt-6 grid grid-cols-4 gap-2">
                {["Mon 09", "Tue 10", "Wed 11", "Thu 12", "Fri 13", "Sat 14", "Mon 16", "Tue 17"].map((d, i) => (
                  <button
                    key={d}
                    className={`rounded-xl px-2 py-3 text-xs font-semibold transition ${
                      i === 2 ? "bg-gradient-gold text-navy" : "bg-white/10 hover:bg-white/20 text-white"
                    }`}
                  >
                    {d}
                  </button>
              ...
              <div className="mt-4 grid grid-cols-3 gap-2">
                {["10:00", "11:30", "14:00", "15:30", "16:30", "17:00"].map((t, i) => (
                  <button key={t} className={`rounded-xl py-2 text-xs font-semibold ${i === 1 ? "bg-white text-navy" : "bg-white/10 hover:bg-white/20 text-white"}`}>
                    {t}
                  </button>
                ))}
              </div>
              <Button className="mt-6 w-full bg-gradient-gold text-navy h-11 rounded-xl font-semibold">Confirm slot</Button>
            </motion.div>
            ...
            >
              ...
              <div className="min-w-0">
                <div className="font-display text-lg font-semibold">{whatsapp.title ?? "Chat on WhatsApp"}</div>
                <div className="text-sm text-muted-foreground">{whatsapp.subtitle ?? "Response time under 15 minutes · 8am – 8pm"}</div>
              </div>
...
function Field({ label, placeholder }: { label: string; placeholder: string }) {
  return (
    <div>
      ...
      <input placeholder={placeholder} className="mt-2 w-full rounded-xl bg-background border border-input px-4 py-3 text-sm outline-none focus:border-primary" />
    </div>
  );
}