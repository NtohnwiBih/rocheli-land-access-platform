...
export default function Resources({ content = {}, articles }: Props) {
  const hero = content.hero ?? {};
  const featured = content.featured ?? {};
  const articlesHeader = content.articles ?? {};
  ...
  return (
    <>
      <Head title="Resources — Rocheli Real Properties">
        <meta name="description" content="Guides, articles, videos and downloads to help you buy, own and grow land in Cameroon." />
        <meta property="og:title" content="Resources — Rocheli Real Properties" />
        <meta property="og:description" content="Everything you need to buy and own land in Cameroon with confidence." />
      </Head>

      <Breadcrumb
        eyebrow={hero.eyebrow ?? "Knowledge hub"}
        title={
          <>
            {hero.title ?? "Everything you need to"}{" "}
            <span className="italic text-gradient-gold">{hero.titleAccent ?? "buy, own, grow."}</span>
          </>
        }
        description={hero.description ?? "Guides, videos, market reports and downloads written by Rocheli's real estate, legal and finance teams."}
      />
      ...
      {featuredItems.length > 0 && (
        <section className="py-16">
          <div className="container-x">
            <SectionHeader eyebrow={featured.eyebrow ?? "Featured"} title={featured.title ?? "Editor's picks this month"} />
            <div className="mt-12 grid gap-6 md:grid-cols-6 auto-rows-[240px]">
              {featuredItems.map((f, i) => {
                const span = i === 0 ? "md:col-span-4 md:row-span-2" : i === 1 ? "md:col-span-2" : i === 2 ? "md:col-span-2" : "md:col-span-3";
                return (
                  <motion.article
                    key={f.id}
      ...
      {latestItems.length > 0 && (
        <section className="py-16 bg-muted/40">
          <div className="container-x">
            <SectionHeader eyebrow={articlesHeader.eyebrow ?? "Latest articles"} title={articlesHeader.title ?? "From the Rocheli desk"} />
